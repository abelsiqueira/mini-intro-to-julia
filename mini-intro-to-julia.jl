...
begin
	...
	struct Dice <: AbstractDice
		sides :: Int
		Dice(n :: Int) = n ≥ 2 ? new(n) : error("number of sides should be ≥ 2")
	end
	struct Constant <: AbstractDice
		value :: Int
	end
	struct OpOnDices{Op,DiceType <: AbstractDice} <: AbstractDice
		dices :: Vector{DiceType}
	end
	struct OpTwoDices{Op,DT1 <: AbstractDice, DT2 <: AbstractDice} <: AbstractDice
		dice1 :: DT1
		dice2 :: DT2
	end
	...
	roll(d :: Constant) = d.value
	Base.:+(dices :: DiceType...) where {DiceType <: AbstractDice} =
		OpOnDices{+,DiceType}([dices...])
	Base.:+(d1 :: DT1, d2 :: DT2) where {DT1 <: AbstractDice, DT2 <: AbstractDice} =
		OpTwoDices{+,DT1,DT2}(d1, d2)
	Base.:+(d :: DT, x :: Int) where {DT <: AbstractDice} =
		d + Constant(x)
	function roll(op_on_dices :: OpOnDices{Op,DiceType}) where {Op, DiceType <: AbstractDice}
		Op(roll.(op_on_dices.dices)...)
	end
	function roll(op_two_dices :: OpTwoDices{Op,DT1,DT2}) where {Op, DT1 <: AbstractDice, DT2 <: AbstractDice}
		Op(roll(op_two_dices.dice1), roll(op_two_dices.dice2))
	end
	...
	function Plots.histogram(d :: AbstractDice, n :: Int)
		result_count = Dict{Int,Int}()
		for _ = 1:n
			value = roll(d)
			if !haskey(result_count, value)
...
let
	...
	for k = 2:20
		...
		composition_array = sort([
			(p, e) for (p, e) in composition
		])
		exp_to_str_dict = Dict(i - 1 => "$x" for (i, x) in enumerate("⁰¹²³⁴⁵⁶⁷⁸⁹"))
		function exp_to_str(e)
			out = ""
			while e > 0
				out = exp_to_str_dict[e % 10] * out
				e = div(e, 10)
			end
			return out
		end
		composition_string = join([
			"$p$(exp_to_str(e))" for (p, e) in composition_array
		], " × ")
		output *= "- $k = $composition_string\n"
	end
...
let
	out = "| value | type | parent type |\n|--:|:--|:--|\n"
	for x in Any[1, 1.0, Float32(1.0), Float16(1.0), 1.0 + 0.0im, true, "1", '1', 0x1, 1//1, (1,), (1,1), Dict(1 => 1), 1 => 1, [1], [1.0], [1 0; 0 1], Int, +, x -> x]
		out *= "| $(repr(x)) | $(typeof(x)) | $(supertype(typeof(x))) |\n"
	end
	Markdown.parse(out)
end
...
let
	fmt(x) = "$x"
	fmt(x::Union{AbstractString,AbstractChar}) = "'$x'"

	where_list = [("date_of_birth", 1986), ("name", "Abel")]

	"SELECT * FROM table WHERE " * join(
		[
			"$col = " * fmt(value)
			for (col, value) in where_list
		],
		" AND ",
	)
end
...
begin
	fun_2d(x) = (x[1] - 1)^2 + 4 * (x[2] - x[1]^2)^2 + 1e-4 * sin(x[1] * exp(x[2]))
	x0 = [-1.5; 1.0]
end
...
let
	...
	for i = 2:max_k
		x = @view X[:, i]
		x .= @view X[:, i-1]
		∇fx = ForwardDiff.gradient(fun_2d, x)
		∇²fx = ForwardDiff.hessian(fun_2d, x)
		x .-= 2.0 .^ ui_lr * (∇²fx \ ∇fx)
	end

	contour(
		range(xrange..., length=N),
		range(yrange..., length=N),
		(x, y) -> fun_2d([x; y]),
		levels = 100,
	)
	title!("Newton method")
...
manifest_format = "2.0"
project_hash = "dcdb4e4fbf34d22e90050b53880a143df7afb3b8"
